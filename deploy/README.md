# Newsroom CMS Deployment Runbook
This is a step-by-step guide to deploy CP Newsroom. Use this to provision environments, perform deployments, or verify system health.
...
* [ ] **Scripts:** The two automation files uploaded to the server: `setup-newsroom-system.sh` and `newsroom.sh`.
* [ ] **Env files:** Variables can be added to `deploy.env` to adjust the `setup-newsroom-system.sh` and `newsroom.sh`.
---
...
2. Execute the environment preparation script using administrative access:
   `sudo ./setup-newsroom-system.sh`

### Phase 2: Start Application
This phase installs application dependencies and starts the newsroom service.
