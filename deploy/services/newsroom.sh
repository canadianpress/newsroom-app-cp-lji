...
REPO_URL="${REPO_URL:-https://github.com/canadianpress/newshub-app-cp.git}"
REPO_REF="${REPO_REF:-cn-develop}"
...
WS_PORT="${WS_PORT:-5150}"
WEB_TIMEOUT="${WEB_TIMEOUT:-30}"
DOMAIN="${DOMAIN:-localhost}"
NEWSAPI_PORT="${NEWSAPI_PORT:-5450}"
MGMTAPI_PORT="${MGMTAPI_PORT:-5550}"
WEB_WORKERS="${WEB_WORKERS:-3}"

# Database
MONGODB_HOST="${MONGODB_HOST:-localhost}"
MONGODB_PORT="${MONGODB_PORT:-27017}"
MONGODB_DB="${MONGODB_DB:-newsroom}"
ELASTICSEARCH_HOST="${ELASTICSEARCH_HOST:-localhost}"
ELASTICSEARCH_PORT="${ELASTICSEARCH_PORT:-9200}"
ELASTICSEARCH_INDEX="${ELASTICSEARCH_INDEX:-newsroom}"
REDIS_HOST="${REDIS_HOST:-localhost}"
REDIS_PORT="${REDIS_PORT:-6379}"
REDIS_DB="${REDIS_DB:-1}"
DEFAULT_TIMEZONE="${DEFAULT_TIMEZONE:-America/Toronto}"

# Content API — MUST point to CMS's content API database
# If CMS is on a separate EC2, set this to the CMS host
CMS_MONGODB_HOST="${CMS_MONGODB_HOST:-${MONGODB_HOST}}"
CMS_MONGODB_PORT="${CMS_MONGODB_PORT:-${MONGODB_PORT}}"
CMS_MONGODB_DB="${CMS_MONGODB_DB:-cms}"

# Content Push
NOTIFICATION_KEY="${NOTIFICATION_KEY:-newsroom}"
PUSH_KEY="${PUSH_KEY:-newsroom}"
...
PR_MANAGER_SIDENAV_ENABLED="${PR_MANAGER_SIDENAV_ENABLED:-true}"
# SAML_LABEL, SAML_COMPANY, SAML_PATH
# AUTH_FIREBASE_AUDIENCE
# CEM_URL, CEM_APIKEY, CEM_PLATFORM, CEM_VERIFY_TLS, CEM_TIMEOUT
# PRODUCTFRUITS_WORKSPACE_CODE
...
preflight_checks() {
    ...
    sudo -n true 2>/dev/null || { log_error "$APP_USER needs sudo access. Run setup-newsroom-system.sh first."; exit 1; }
...
render_template() {
    ...
    for var in APP_NAME APP_USER DEPLOY_DIR SERVER_DIR CLIENT_DIR ENV_FILE \
               WEB_PORT WS_PORT NEWSAPI_PORT MGMTAPI_PORT DOMAIN LOG_DIR \
               WEB_WORKERS WEB_TIMEOUT DEFAULT_TIMEZONE \
               ELASTICSEARCH_INDEX NOTIFICATION_KEY PUSH_KEY; do
...
setup_env_file() {
    ...
    local client_url websocket_url
    ...
    # Build database URLs
    local mongo_uri="mongodb://${MONGODB_HOST}:${MONGODB_PORT}/${MONGODB_DB}"
    local es_url="http://${ELASTICSEARCH_HOST}:${ELASTICSEARCH_PORT}"
    local redis_url="redis://${REDIS_HOST}:${REDIS_PORT}/${REDIS_DB}"

    # Content API — use same database as MONGO_URI (upstream default).
    # CMS pushes content via HTTP, not shared DB access.
    local contentapi_mongo_uri="$mongo_uri"

    # Build optional vars block
    local optional_vars=""
    for var in SAML_LABEL SAML_COMPANY SAML_PATH AUTH_FIREBASE_AUDIENCE \
               CEM_URL CEM_APIKEY CEM_PLATFORM CEM_VERIFY_TLS CEM_TIMEOUT \
               PRODUCTFRUITS_WORKSPACE_CODE PR_MANAGER_SIDENAV_ENABLED \
               PR_MANAGER_SIDENAV_URL MAX_CONTENT_LENGTH; do
        if [[ -n "${!var:-}" ]]; then
...
"
    ...
    render_template "$TEMPLATE_DIR/newshub.env.template" \
...
initialize_database() {
    ...
    source "$ENV_FILE"
    set +a
    python3 manage.py initialize_data || log_warn "initialize_data may have partial failures (this is often OK)"
    ...
    python3 manage.py create_user \
        ...
        "${ADMIN_LASTNAME:-User}" \
        true || log_warn "Admin user may already exist"
...
create_procfile() {
    ...
    install_template "newsroom.procfile.template" "${SERVER_DIR}/Procfile"
...
configure_nginx() {
    ...
    install_template "newshub.nginx.conf.template" "/etc/nginx/conf.d/${APP_NAME}.conf"
...
show_status() {
    ...
    echo ""
    log_info "Port: $WEB_PORT | Commit: $(git -C "$DEPLOY_DIR" rev-parse --short HEAD)"
    echo ""
